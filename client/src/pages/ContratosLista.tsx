...
// Mock contracts for display when DB is empty
const mockContracts = [
    {
        id: "mock-1",
...
export default function ContratosLista() {
    ...
    return (
        <DashboardLayout>
            ...
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8 animate-enter">
                <div>
                    <div className="flex items-center gap-2 mb-1">
                        <Link href="/">
                            <a className="text-sm text-gray-500 hover:text-pifc-primary transition-colors">Dashboard</a>
                        </Link>
                    ...
                    <p className="text-sm text-gray-600 mt-1">Visualize e gerencie todos os contratos ativos do Ministério.</p>
                </div>
            ...
            {/* Stats */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8 animate-enter" style={{ animationDelay: "50ms" }}>
                {[
                    { icon: "assignment", color: "bg-pifc-primary/10 text-pifc-primary", label: "Total de Contratos", value: stats.total },
                    { icon: "check_circle", color: "bg-ds-success/10 text-ds-success", label: "Ativos / Em Análise", value: stats.active },
                    { icon: "edit_note", color: "bg-gray-100 text-gray-500", label: "Rascunhos", value: stats.draft },
                    { icon: "warning", color: "bg-ds-error/10 text-ds-error", label: "Com Alertas", value: stats.alerts },
                ].map((s) => (
                    <div key={s.label} className="bg-white rounded-xl p-5 border border-gray-200 shadow-sm">
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg ${s.color.split(" ")[0]}`}>
                                <span className={`material-icons-outlined text-xl ${s.color.split(" ")[1]}`}>{s.icon}</span>
                            </div>
                            <div>
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden animate-enter" style={{ animationDelay: "100ms" }}>
                    ...
                    <div className="p-6">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                            {filteredContracts.map((contract) => {
                                ...
                                return (
                                    <Link key={contract.id} href="/painel-contrato">
                                        <a className="group block bg-white rounded-xl border border-gray-200 p-5 hover:border-pifc-primary/50 hover:shadow-md transition-all duration-200 cursor-pointer">
                                            <div className="flex items-start justify-between mb-3">
                                                ...
                                                <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-semibold border shrink-0 ml-3 ${statusCfg.bg}`}>
                                            ...
                                            <div className="flex items-center gap-6 mb-3">
                                                <div className="flex items-center gap-2">
                                                    <div className="relative w-10 h-10">
                                                        ...
                                                        <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-gray-700">{contract.healthScore}</span>
                                                    </div>
                                                    <div>
                                                        <p className="text-[10px] text-gray-400 uppercase tracking-wide font-medium">Saúde</p>
                                                        <p className={`text-xs font-semibold ${getHealthColor(contract.healthScore)}`}>
                                                            {contract.healthScore >= 80 ? "Bom" : contract.healthScore >= 60 ? "Atenção" : "Crítico"}
                                                        </p>
                                                    </div>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] text-gray-400 uppercase tracking-wide font-medium">SLA</p>
                                                    <p className="text-sm font-bold text-gray-800">{contract.slaScore}%</p>
                                                </div>
                                                <div>
                                                    <p className="text-[10px] text-gray-400 uppercase tracking-wide font-medium">Valor</p>
                                                    <p className="text-sm font-bold text-gray-800">R$ {(Number(contract.value) / 1000000).toFixed(1)}M</p>
                                                </div>
                                                <div className="hidden sm:block">
                                                    <p className="text-[10px] text-gray-400 uppercase tracking-wide font-medium">Data</p>
                                                    <p className="text-sm text-gray-600">
                                                        {(() => { try { return format(new Date(contract.contractDate), "dd/MM/yyyy"); } catch { return contract.contractDate; } })()}
                                                    </p>
                                                </div>
                                            </div>
                        ...
                        {filteredContracts.length === 0 && (
                            <div className="text-center py-16">
                                <span className="material-icons-outlined text-5xl text-gray-300 mb-3 block">search_off</span>
                                <h4 className="text-lg font-semibold text-gray-700 mb-1">Nenhum contrato encontrado</h4>
                ...
                <div className="p-4 border-t border-gray-200 flex items-center justify-between">
                    <span className="text-sm text-gray-500">Mostrando {filteredContracts.length} de {allContracts.length} contratos</span>
                    <div className="flex gap-2">
                        ...
                        <button className="px-3 py-1 border border-gray-300 rounded text-sm text-gray-600 hover:bg-gray-50">Próximo</button>
                    </div>
                </div>
            </div>
        </DashboardLayout>
